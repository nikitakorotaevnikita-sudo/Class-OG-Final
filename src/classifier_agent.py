...
Найди в тексте ВСЕ адреса и для каждого определи по контексту, кому он принадлежит.
Заявителю НЕ принадлежат: адрес должностного лица или представителя органа власти;
адрес самого органа власти или государственной организации; адрес коммерческой
организации — банка, управляющей компании, застройщика, подрядчика.
Признаки адреса заявителя: стоит рядом с его именем и фамилией; в блоке реквизитов
заявителя или в поле «контактные данные»; сопровождается словами «мой», «обратный
адрес», «обратная связь», «связаться со мной», «ответ прошу направить на»,
«написать ответ на»; стоит в подписи обращения.
НЕ бери адрес из шапки сопроводительного письма и из приложений к обращению —
протоколов собраний, актов, служебных записок, копий переписки: там адреса
организаций, а не заявителя.
Верни адрес ДОСЛОВНО как в тексте. Если адреса заявителя нет — null; если
подходящих несколько — первый по тексту.
...
{{
  ...
  "applicant_email": "email ЗАЯВИТЕЛЯ из текста, либо null",
...
class ClassifierAgent:
    ...
    def classify(
        ...
        # найденными в тексте адресами.
        applicant_email = extract_applicant_email(
            appeal_text, llm_hint=llm_result.get("applicant_email"))
