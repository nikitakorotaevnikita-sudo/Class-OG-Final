...
def test_positive_noun_forms():
    """Отглагольные существительные — частая форма в теме/заголовке письма."""
    positives = [
        "Отмена обращения SED260005101 № 23-44-000292/26",
        "Отмена обращения",
        "Отзыв обращения, направленного ранее",
        "Прекращение рассмотрения обращения",
        "Аннулирование заявления",
        "Отменяю своё заявление",
        "Прошу отменить обращение",
        "Отмена ранее направленного запроса",
    ]
    for text in positives:
        assert detect_withdrawal_markers(text), f"должно детектироваться: {text}"
...
def test_negative_similar_words_but_other_meaning():
    ...
    negatives = [
        ...
        "Прошу разобраться с отменой автобусного маршрута №15",
        "Незаконная отмена льготы на проезд, прошу восстановить",
    ]
